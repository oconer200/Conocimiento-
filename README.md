...
-- 1. V_EstadoActualParcelas: Muestra la situación actual de todas las parcelas incluyendo cultivo, fecha de siembra y cosecha estimada.
...
SELECT 
    ...
    s.FechaSiembra,
    s.FechaEstCosecha,
    s.RendimientoEsperado,
    sr.nombre AS sistema_riego
FROM 
    Parcelas p
JOIN 
    Fincas f ON p.id_finca = f.id_finca
JOIN 
    Siembras s ON p.id_parcela = s.ParcelaID
JOIN 
    Cultivos c ON s.CultivoID = c.id_cultivo
JOIN 
    Sistemas_Riego sr ON p.id_riego = sr.id_riego;
    

-- 2. V_InventarioInsumos: Presenta el inventario actualizado de insumos con información de stock, valor y ubicación.
...
SELECT 
    ...
    (i.stock * i.precio) AS valor_total_inventario,
    a.nombre AS almacen,
    a.ubicacion,
    i.FechaCaducidad
FROM 
    Insumos i
JOIN 
    Tipos_Insumo ti ON i.id_tipo_insumo = ti.id_tipo_insumo
JOIN 
    Unidades_Medida um ON i.id_unidad = um.id_unidad
JOIN 
    Almacenes a ON i.id_almacen = a.id_almacen;
    
    
-- 3. V_CalendarioMantenimientoMaquinaria: Calendario de mantenimientos programados para la maquinaria.
...
SELECT 
    ...
    ma.Marca,
    ma.Modelo,
    ma.Horometro AS Horas_Actuales,
...
FROM 
    Maquinaria ma
LEFT JOIN 
    Mantenimientos m ON ma.MaquinariaID = m.MaquinariaID
LEFT JOIN 
    Empleados e ON m.TecnicoID = e.dni
ORDER BY 
    m.Fecha DESC;


-- 4. V_RendimientoCultivos: Estadísticas de rendimiento por cultivo, parcela y temporada.
...
SELECT 
    ...
    s.SiembraID,
    s.FechaSiembra,
    co.fecha_fin AS FechaCosechaReal,
    s.RendimientoEsperado,
    co.cantidad AS RendimientoReal,
    (co.cantidad - s.RendimientoEsperado) AS Diferencia,
    cal.nombre AS CalidadObtenida,
    mc.nombre AS MetodoCosecha
FROM 
    Cosechas co
JOIN 
    Siembras s ON co.id_siembra = s.SiembraID
JOIN 
    Cultivos c ON s.CultivoID = c.id_cultivo
JOIN 
    Parcelas p ON s.ParcelaID = p.id_parcela
JOIN 
    Calidades cal ON co.id_calidad = cal.id_calidad
JOIN 
    Metodos_Cosecha mc ON co.id_metodo_cosecha = mc.id_metodo_cosecha;


-- 5. V_AsignacionEmpleados: Muestra las asignaciones actuales de los empleados por parcela y tarea.
...
FROM 
    Asignaciones a
JOIN 
    Empleados e ON a.dni_empleado = e.dni
JOIN 
    Parcelas p ON a.id_parcela = p.id_parcela
JOIN 
    Tareas t ON a.id_tarea = t.id_tarea
JOIN 
    Especialidades esp ON e.id_especialidad = esp.id_especialidad;
...
CREATE PROCEDURE RegistrarSiembra(
    ...
    IN p_densidad DECIMAL(10,2),
    IN p_rendimiento_esp DECIMAL(10,2)
)
BEGIN
    DECLARE v_temp_finca DECIMAL(5,2);
    DECLARE v_temp_optima DECIMAL(5,2);
    DECLARE v_fecha_estimada DATE;
    DECLARE v_dias_crecimiento INT;

    
    SELECT f.TempPromedio INTO v_temp_finca
    FROM Parcelas p
    JOIN Fincas f ON p.id_finca = f.id_finca
    WHERE p.id_parcela = p_id_parcela;

    
    SELECT TempOptima, tiempo_crecimiento INTO v_temp_optima, v_dias_crecimiento
    FROM Cultivos
    WHERE id_cultivo = p_id_cultivo;

    
    IF ABS(v_temp_finca - v_temp_optima) > 5.00 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Error: La temperatura de la finca no es apta para este cultivo.';
    ELSE
        
        SET v_fecha_estimada = DATE_ADD(p_fecha_siembra, INTERVAL v_dias_crecimiento DAY);

        
        INSERT INTO Siembras (
            SiembraID, ParcelaID, CultivoID, FechaSiembra, 
            DensidadSiembra, FechaEstCosecha, RendimientoEsperado
        ) VALUES (
            p_id_siembra, p_id_parcela, p_id_cultivo, p_fecha_siembra, 
            p_densidad, v_fecha_estimada, p_rendimiento_esp
        );
        
        SELECT 'Siembra registrada con éxito' AS Resultado;
    END IF;
END //
...
-- 2. PlanificarCosecha: Planifica una cosecha basada en el tiempo de crecimiento del cultivo y la fecha de siembra.
...
CREATE PROCEDURE PlanificarCosecha(
    IN p_id_cosecha INT,
    IN p_id_siembra INT,
    IN p_id_metodo_cosecha INT,
    IN p_id_calidad INT
)
BEGIN
    DECLARE v_fecha_siembra DATE;
    DECLARE v_dias_crecimiento INT;
    DECLARE v_fecha_inicio_estimada DATE;
    DECLARE v_fecha_fin_estimada DATE;

    
    SELECT s.FechaSiembra, c.tiempo_crecimiento 
    INTO v_fecha_siembra, v_dias_crecimiento
    FROM Siembras s
    JOIN Cultivos c ON s.CultivoID = c.id_cultivo
    WHERE s.SiembraID = p_id_siembra;

   
    
    SET v_fecha_inicio_estimada = DATE_ADD(v_fecha_siembra, INTERVAL v_dias_crecimiento DAY);
   
    SET v_fecha_fin_estimada = DATE_ADD(v_fecha_inicio_estimada, INTERVAL 5 DAY);
    ...
    SELECT CONCAT('Cosecha planificada del ', v_fecha_inicio_estimada, ' al ', v_fecha_fin_estimada) AS Resultado;
END //
...
-- 3. AsignarTrabajadores: Asigna empleados a tareas específicas en una parcela verificando su disponibilidad.
...
BEGIN
    ...
    FROM Asignaciones 
    WHERE dni_empleado = p_dni_empleado AND fecha = p_fecha;

    
    IF v_ocupado > 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Error: El empleado ya tiene una asignación para esta fecha.';
    ELSE
        
        INSERT INTO Asignaciones (
            id_asignacion, fecha, horas, dni_empleado, 
            id_parcela, id_tarea, Observaciones
        ) VALUES (
            p_id_asignacion, p_fecha, p_horas, p_dni_empleado, 
            p_id_parcela, p_id_tarea, p_observaciones
        );
        
        SELECT 'Empleado asignado correctamente' AS Resultado;
    END IF;
END //
...
-- 4. RegistrarAnalisisSuelo: Registra un análisis de suelo y genera recomendaciones automáticas basadas en los valores registrados.
...
CREATE PROCEDURE RegistrarAnalisisSuelo(
    ...
    IN p_p DECIMAL(10,2),
    IN p_k DECIMAL(10,2),
    IN p_mo DECIMAL(10,2),
    IN p_ce DECIMAL(10,2),
    IN p_laboratorio VARCHAR(100)
)
BEGIN
    DECLARE v_recomendacion TEXT DEFAULT 'Suelo en condiciones estables. ';

   
    IF p_ph < 5.5 THEN
        SET v_recomendacion = CONCAT(v_recomendacion, 'Suelo muy ácido: Se recomienda aplicar cal agrícola. ');
    ELSEIF p_ph > 7.5 THEN
        SET v_recomendacion = CONCAT(v_recomendacion, 'Suelo alcalino: Se recomienda aplicar azufre elemental o materia orgánica. ');
    END IF;

   
    IF p_n < 20.00 THEN
        SET v_recomendacion = CONCAT(v_recomendacion, 'Bajo nivel de Nitrógeno: Aplicar fertilizante nitrogenado (Urea o Sulfato de Amonio). ');
    END IF;

    
    IF p_mo < 2.00 THEN
        SET v_recomendacion = CONCAT(v_recomendacion, 'Materia orgánica deficiente: Incorporar compost o abonos verdes.');
    END IF;

    
    INSERT INTO Analisis_Suelo (
        ParcelaID, FechaMuestreo, pH, NutrienteN, NutrienteP, 
        NutrienteK, MateriaOrganica, ConductividadElectrica, 
        Laboratorio, Recomendaciones
    ) VALUES (
        p_parcela_id, p_fecha, p_ph, p_n, p_p, 
        p_k, p_mo, p_ce, p_laboratorio, v_recomendacion
    );

    SELECT v_recomendacion AS 'Recomendacion Generada';
END //

DELIMITER ;


-- 5. CalcularRendimientoCosecha: Calcula el rendimiento de una cosecha comparándolo con el esperado y registrando las diferencias.
...
BEGIN
    ...
    DECLARE v_real DECIMAL(10,2);
    DECLARE v_diferencia DECIMAL(10,2);
    DECLARE v_porcentaje DECIMAL(5,2);
    DECLARE v_cultivo VARCHAR(50);

    
    SELECT s.RendimientoEsperado, c.cantidad, cu.nombre_comun
    INTO v_esperado, v_real, v_cultivo
    FROM Cosechas c
    JOIN Siembras s ON c.id_siembra = s.SiembraID
    JOIN Cultivos cu ON s.CultivoID = cu.id_cultivo
    WHERE c.id_cosecha = p_id_cosecha;

    
    IF v_real IS NULL OR v_real = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Error: La cosecha seleccionada no tiene una cantidad real registrada para comparar.';
    ELSE
        
        SET v_diferencia = v_real - v_esperado;
        SET v_porcentaje = (v_real / v_esperado) * 100;

        
        SELECT 
            v_cultivo AS Cultivo,
            v_esperado AS Rendimiento_Esperado,
            v_real AS Rendimiento_Real,
            v_diferencia AS Diferencia_Absoluta,
            CONCAT(v_porcentaje, '%') AS Porcentaje_Eficiencia,
            CASE 
                WHEN v_porcentaje >= 100 THEN 'Excelente: Meta superada'
                WHEN v_porcentaje >= 85 THEN 'Bueno: Dentro del margen'
                ELSE 'Alerta: Rendimiento bajo lo esperado'
            END AS Diagnostico;
    END IF;
END //
